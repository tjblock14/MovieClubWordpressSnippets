...
document.addEventListener("DOMContentLoaded", () => 
{
    ...
    modalStyle.textContent = `
        ...
        .mc-modal.open, .mc-modal-backdrop.open
        {
            display : flex;
        ...
        <div class = "mc-card" >
            <div class = "mc-hd" >
                <div>
                    ...
                    <div id = "mc-subtitle" style = "font-size : 12px; opacity : .75; margin-top : 4px;" >
                    </div>
                </div>
                <button id = "mc-close" class = "mc-btn" >
                    Close
                </button>
            </div>
            <div class = "mc-bd">
                ...
                <div class = "mc-viewonly" style = "display : none;" >
                    <div style = "margin-bottom : 14px;" >
                        ...
                        <div id = "mc-view-rating" style = "font-size : 20px; font-weight : 700;" >
                        </div>
                    </div>
                    <div>
                        <div style = "font-size : 12px; opacity : .8; margin-bottom : 6px;" >
                        ...
                        <div id = "mc-view-review" class = "view-only-rating-popout-window" >
                        </div>
                    </div>
                </div>
                ...
                <div class = "mc-editable" style = "display : none;" >
                    ...
                    <input
                        id = "mc-rating" type = "number" min = "0" max = "10" step = "0.1" class = "popout-window-editable-input" 
                    />

                    <label style = "display : block; font-size : 12px; opacity : .8; margin : 14px 0 6px;" >
                        Justification
                    </label>
                    <textarea id = "mc-textarea" class = "mc-txt" placeholder = "Type your justification..." >
                    </textarea>

                    <div id = "mc-hint" style = "margin-top : 10px; font-size : 12px; opacity : .75;" >
                    </div>
                </div>
            </div>

            <div class = "mc-ft" >
                <button id = "mc-cancel" class = "mc-btn" >
                    Close
                </button>
                <button id = "mc-save" class = "mc-btn primary" >
                    Save
                </button>
            </div>
        </div>`;
...
function syncJustificationMarker(cell)
{
    ...
    if(!justification)
    {
        ...
        if(row)
        {
            ...
            if(reviewer)
            {
                ...
                if(reviewCell)
                {
                    justification = (reviewCell.textContent || "").trim();
                }
            }
        }
    }
    ...
    if(justification)
    {
        cell.classList.add("has-justification");
    }
    else
    {
        cell.classList.remove("has-justification");
    }
}
...
async function saveRatingAndJustification(cell, ratingNum, justification)
{
    ...
    if(!token)
    {
        toast("You must be logged in to update a review.", 2200); 
    ...
    if(existingReviewId)
    {
        ...
        if(reviewType === "tv")
        {
            ...
            {
                payload.tv_season_type = parseInt(itemId);
            }
            if(targetType === "episode")
            {
                payload.tv_episode_type = parseInt(itemId);
            }
        }
    }
    else
    {
        ...
        if(reviewType === "tv")
        {
            ...
            if(coupleSlug)
            {
                payload.couple_slug = coupleSlug;
            }
...
function openRatingReviewModalFromCell(cell)
{
    ...
    if(!review)
    {
        ...
        if(row)
        {
            ...
            if(reviewCell)
            {
                review = (reviewCell.textContent || "").trim();
            }
        }
    }
    ...
    if(viewRating)
    {
        ...
        if(ratingColor)
        {
            viewRating.style.color = ratingColor;
        }
    }

    if(viewReview)
    {
        viewReview.textContent = review || "—";
    }
    ...
    if(ratingInput)
    {
        ...
        if(ratingColor)
        {
            ratingInput.style.color = ratingColor;
        }
    }
    if(reviewTextarea)
    {
