...
add_shortcode('tnt_table', function() {
   ...
   <table style="border-collapse: collapse; min-width: 1400px; font-size: 14px; border: 2px solid black;">
    ...
    foreach ($movies as $movie) {
        ...
        $review_id_trevor = $movie['reviews']['trevor']['id'] ?? '';
        $review_id_taylor = $movie['reviews']['taylor']['id'] ?? '';

        $trevor_color = color_rating_cell($trevor_rating);
        $html .= '<td class="rating-cell" data-reviewer="trevor" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_trevor . '" data-rating="' . esc_attr($trevor_rating) . '" style="background-color: ' 
        . $trevor_color . ';">' . esc_html($trevor_rating) . '</td>';

$html .= '<td class="review-cell" data-reviewer="trevor" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_trevor . '">' . esc_html($trevor_review) . '</td>';

$taylor_color = color_rating_cell($taylor_rating);
$html .= '<td class="rating-cell" data-reviewer="taylor" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_taylor . '" data-rating="' . esc_attr($taylor_rating) . '" style="background-color: ' 
        . $taylor_color . ';">' . esc_html($taylor_rating) . '</td>';

$html .= '<td class="review-cell" data-reviewer="taylor" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_taylor .'">' . esc_html($taylor_review) . '</td>';
