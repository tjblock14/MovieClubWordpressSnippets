...
add_shortcode('sb_table', function() {

    // Updated to call the correct endpoint (must match the WPGetAPI endpoint name in WP settings)
    $data = wpgetapi_endpoint('movie_club_database_api', 'sb_reviews', array(
        'return' => 'body',
        'debug' => false,
        'cache' => false
    ));
   ...
   <table style="border-collapse: collapse; min-width: 1400px; font-size: 14px; border: 2px solid black;">
        <thead>
            <tr>
                ...
                <th style="width: 100px; font-size: 18px; font-weight: bold; border-collapse: collapse; border: 1px solid black;">Sierra Rating</th>
                <th style="width: 600px; font-size: 18px; font-weight: bold; border-collapse: collapse; border: 1px solid black;">Sierra Review</th>
                <th style="width: 100px; font-size: 18px; font-weight: bold; border-collapse: collapse; border: 1px solid black;">Benett Rating</th>
                <th style="width: 600px; font-size: 18px; font-weight: bold; border-collapse: collapse; border: 1px solid black;">Benett Review</th>
            </tr>
        </thead>
        <tbody>';
    ...
    foreach ($movies as $movie) {
        ...
        // Capitalized keys ("Sierra", "Benett") match the JSON structure returned by the API
        $sierra_rating  = $movie['reviews']['Sierra']['rating'] ?? '';
        $sierra_review  = $movie['reviews']['Sierra']['review'] ?? '';
        $benett_rating  = $movie['reviews']['Benett']['rating'] ?? '';
        $benett_review  = $movie['reviews']['Benett']['review'] ?? '';

        // Get the ID of the review so we have it when a POST or PATCH is sent to the API
        $review_id_sierra = $movie['reviews']['sierra']['id'] ?? '';
        $review_id_benett = $movie['reviews']['benett']['id'] ?? '';

        $sierra_color = color_rating_cell($sierra_rating);
        $html .= '<td class="rating-cell" data-reviewer="sierra" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_sierra . '" data-rating="' . esc_attr($sierra_rating) . '" style="background-color: ' 
                . $sierra_color . ';">' . esc_html($sierra_rating) . '</td>';

        $html .= '<td class="review-cell" data-reviewer="sierra" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_sierra . '">' . esc_html($sierra_review) . '</td>';


        $benett_color = color_rating_cell($benett_rating);
        $html .= '<td class="rating-cell" data-reviewer="benett" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_benett . '" data-rating="' . esc_attr($benett_rating) . '" style="background-color: ' 
                . $benett_color . ';">' . esc_html($benett_rating) . '</td>';

        $html .= '<td class="review-cell" data-reviewer="benett" data-id="' . $movie_id . '" data-movie-title="' . esc_attr($movie['title']) . '" data-review-id="' . $review_id_benett .'">' . esc_html($benett_review) . '</td>';
