   ...
   <table style="border-collapse: collapse; min-width: 1400px; font-size: 14px; border: 2px solid black;">
    ...
    foreach ($movies as $movie) {
	   ...
	   $html .= '<td style="width: 200px; text-align: center; vertical-align: middle; font-size: 14px; font-weight: bold; border-collapse: collapse; border: 1px solid black;">' . esc_html(safe_implode($movie['genres'])) . '</td>';
        ...
        $trevor_color = color_rating_cell($trevor_rating);
	    $html .= '<td class="rating-cell" data-reviewer="trevor" data-id="' . $movie_id . '" data-rating="' . esc_attr($trevor_rating) . '" style="background-color: ' 
                . $trevor_color . '; color: white; font-weight: bold; font-size: 14px; text-align: center; vertical-align: middle; width: 100px; 
                border-collapse: collapse; border: 1px solid black;">' . esc_html($trevor_rating) . '</td>';

        $html .= '<td class="review-cell" data-reviewer="trevor" data-id="' . $movie_id . '" style="color: white; font-size: 14px; text-align: center; 
                vertical-align: middle; width: 600px; border: 1px solid black;">' . esc_html($trevor_review) . '</td>';

        $taylor_color = color_rating_cell($taylor_rating);
		$html .= '<td class="rating-cell" data-reviewer=taylor" data-id="' . $movie_id . '" data-rating="' . esc_attr($taylor_rating) . '" style="background-color: ' 
                . $taylor_color . '; color: white; font-weight: bold; font-size: 14px; text-align: center; vertical-align: middle; width: 100px; 
                border-collapse: collapse; border: 1px solid black;">' . esc_html($taylor_rating) . '</td>';

        $html .= '<td class="review-cell" data-reviewer="taylor" data-id="' . $movie_id . '" style = "color: white; font-size: 14px; text-align: center; 
                vertical-align: middle; width: 600px; border: 1px solid black;">' . esc_html($taylor_review) . '</td>';
